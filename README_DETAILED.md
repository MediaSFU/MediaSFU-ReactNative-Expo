# MediaSFU React Native Expo SDK — detailed reference

> Temporary local or workspace SDK references are for validation only. Replace them with the next official minor package release before publishing an application.

> This extended manual preserves legacy examples for migration context. Use [README.md](README.md) and [HEADLESS_GUIDE.md](HEADLESS_GUIDE.md) for the current production-safe path; older client-side credential snippets are local-development placeholders only.
...
**Interested in getting just the media stream of a specific participant?** You can now easily retrieve individual participant streams using `getParticipantMedia()` method. [Learn more →](#media-device-and-stream-utility-methods)
